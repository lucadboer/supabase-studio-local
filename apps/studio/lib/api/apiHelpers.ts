...
import { getEncryptedConnectionHeader } from '@/lib/local-connection'
...
export function constructHeaders(headers: { [prop: string]: any }) {
  if (headers) {
    let xConnEncrypted = headers['x-connection-encrypted']
    if (!IS_PLATFORM && !xConnEncrypted && headers.cookie) {
      const injected = getEncryptedConnectionHeader({ headers: { cookie: headers.cookie } })
      if (injected) xConnEncrypted = injected
    }
    const cleansedHeaders = {
      ...
      'x-connection-encrypted': xConnEncrypted,
    } as any
